...
<body>
    ...
    <form method="post" action="search_job_processing.php">
        ...
        <!-- Search input -->
            <input type="text" name="searchJob" id="searchJob" placeholder="Enter job name" class="search-input">
    ...
    <div style="width: 25%; float:left">
        <?php
            function searchJobs($searchJob, $conn) {
                // Include database connection
            
                // Query to search for companies
                $sql = "SELECT * FROM jobs WHERE jobTitle LIKE '%$searchJob%'";
                $result = mysqli_query($conn, $sql);
            ...
            searchJobs($searchForJob, $conn);
        ?>
    </div>
    <div style="width: 75%; float:right">
        <?php
            $sql = "SELECT * FROM jobs WHERE jobTitle LIKE '%$searchForJob%'";
            $result = mysqli_query($conn, $sql);
            if (mysqli_num_rows($result) > 0) {
